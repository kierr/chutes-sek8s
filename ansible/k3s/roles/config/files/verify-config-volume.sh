...
EXPECTED_LABEL="tdx-config"
LOG_TAG="verify-config-volume"
...
log_info "=== Config Volume Verification Started ==="
...
if [ -z "$DEVICE" ]; then
    log_error "Config device with label '$EXPECTED_LABEL' not found"
    log_error "Available block devices:"
    blkid | while read line; do
        log_error "  $line"
    done
    log_error "The config volume was not attached to this VM or has wrong label"
    log_error "Shutting down immediately to prevent boot with missing config"
    sync
    shutdown -h now
    exit 1
fi

log_info "Config device found: $DEVICE"
...
if [ "$FS_TYPE" != "ext4" ]; then
    log_error "Config device has wrong filesystem type: $FS_TYPE (expected ext4)"
    log_error "The config volume must be formatted with ext4"
    log_error "Shutting down immediately"
    sync
    shutdown -h now
    exit 1
...
if [ "$FS_LABEL" != "$EXPECTED_LABEL" ]; then
    log_error "Config device has wrong label: '$FS_LABEL' (expected '$EXPECTED_LABEL')"
    log_error "The config volume must have the label '$EXPECTED_LABEL'"
    log_error "Shutting down immediately"
    sync
    shutdown -h now
    exit 1
...
if tune2fs -l "$DEVICE" 2>/dev/null | grep -q "needs checking"; then
    ...
    if ! e2fsck -p "$DEVICE"; then
        ...
        log_error "The config volume may be corrupt"
        log_error "Shutting down immediately"
        sync
        shutdown -h now
        exit 1
...
log_info "=== Config Volume Verification Complete ==="
