...
class MetricsCollector:
    ...
    def record_admission_decision(
        self, allowed: bool, resource_kind: str, operation: str, duration: float
    ):
    ...
    def export_prometheus(self) -> str:
        ...
        for kind_decision, count in self.admission_by_kind.items():
            ...
            if len(parts) == 2:
                kind, decision = parts
                lines.append(
                    f'admission_requests_by_kind_total{{kind="{kind}",decision="{decision}"}} {count}'
                )
        ...
        for op_decision, count in self.admission_by_operation.items():
            ...
            if len(parts) == 2:
                ...
                lines.append(
                    f'admission_requests_by_operation_total{{operation="{operation}",decision="{decision}"}} {count}'
                )
        ...
        if self.admission_duration_count > 0:
            ...
            lines.append(
                f"admission_request_duration_seconds_count {self.admission_duration_count}"
            )
